...
const CategoryResults = ({ topCareerMatches }) => {
    const [hoveredBar, setHoveredBar] = useState(null);
  
    // Get top 3 categories based on passed top careers
    const getTopCategories = () => {
      const categoryCounts = {};
  
      topCareerMatches.forEach((career) => {
    ...
    const filteredBars = bars.filter((bar) =>
      topCategories.includes(bar.name)
    );
    ...
    return (
      ...
      >
        <div style={{ flex: "1.5", marginRight: "30px", marginTop: "-22px" }}>
          ...
          <p
            style={{
              fontSize: "clamp(.5rem, 2vw, 1.5rem)",
              color: "#311106",
            }}
          >
            {hoveredBar
              ? hoveredBar.description
              : "These career categories match your interests the most. Hover over the bars to learn more about each category and their common educational pathways. Use these categories to filter for jobs that best fit your interests on the explore page!"}
          </p>
          {!hoveredBar && (
            <div
              style={{
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isHovered ? "0 4px 8px rgba(0,0,0,0.4)" : "0 4px 8px rgba(0,0,0,0.2)",
        cursor: "pointer",
      }}
    >
